...
def user_profile_sethandle_v1(token, handle_str):
    ...
    # Loop to find the handle and replace by the new one
    idx = 0
...
def user_profile_setname_v1(token, name_first, name_last):
    ...
    # Loop to find the names and replace by the new ones
    idx = 0
...
def user_profile_setemail_v1(token, email):
    ...
    # Loop to find the email and replace by the new one
    idx = 0
...
# Check the length of handle_str (3,20)
def check_len(handle_str):
    if len(handle_str) in range(3, 21):
        pass
    else:
        raise InputError(description='Invalid User Name')

# Check if it its alphanumeric
...
# Check the length of first names (1-50)
def check_name_first_len(first_names):
    if len(first_names) in range(1, 51):
        pass
    else:
        raise InputError(description='Invalid User Name')
