...
def dm_create_v1(token, u_ids):
    ...
    all_users = u_ids.append(int(token))
    
    #double check if all members are tokens for users or user_ids
    dm_id = len(store['dms']['dm_id']) + 1

    store['dms']['dm_id'].append(dm_id)
    store['dms']['dm_name'].append(dm_name)
    store['dms']['owner_user_id'].append(int(token))
...
def dm_list_v1(token):
    '''
    Creates a list of dictiomaries that contain the name and ID of all DMs
...
def dm_remove_v1(token, dm_id):
    ...
    check_original_dm(token, dm_id, store)
...
def dm_leave_v1(token, dm_id):
    ...
    store['dms']['all_members'][index].remove(int(token))
    if store['dms']['owner_user_id'][index] == int(token):
...
def check_user_in_dm(token, dm_id, store):
    index = index_from_dm_id(dm_id, store)

    if int(token) not in store['dms']['all_members'][index]:
        raise AccessError('Given User is not a memeber of DM')
    else:
        return

# Checks if the authorised user (token) is the original owner of the DM
def check_original_dm(token, dm_id, store):
    index = index_from_dm_id(dm_id, store)

    if int(token) != store['dms']['owner_user_id'][index]:
